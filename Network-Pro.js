/**
 * 📌 桌面小组件: 🛡️ 网络诊断雷达 (全栈解锁 Pro 版 - CF 极速无缝无缓存版)
 * 🎨 全面移除持久化缓存，出口全量升级为 Cloudflare 官方多栈骨干网元数据引擎
 * 文件名: Network-Pro.js
 */
export default async function(ctx) {
  ...
  // 3. 实时并行请求核心：引入 Cloudflare 骨干网探测
  ...
  // 🛠️ 核心升级：通过 Cloudflare 边缘计算节点直接拉取高精度的代理出口地理数据与双栈 IP
  const fetchCFMeta = async () => {
    try {
      const res = await ctx.http.get('https://speed.cloudflare.com/meta', { timeout: 3500 });
  ...
  // 4. 第一阶段：全量轻量级并发（将云探测融入首轮，速度直接拉满）
  const [localData, cfMeta, localDelay, proxyDelay] = await Promise.all([
    fetchLocal(), fetchCFMeta(), fetchLocalDelay(), fetchProxyDelay()
  ]);

  // 5. 第二阶段：欺诈分探测与解锁同步高并发（完全抛弃缓存，始终保持最新状态）
  ...
  const [purData, rNF, rDP, rTK, rGPT, rCL, rGM] = await Promise.all([
    fetchPurityFull(),
    checkNetflix(), checkDisney(), checkTikTok(), checkChatGPT(), checkClaude(), checkGemini()
  ]);

  // 6. 双层汉化高级映射字典清洗
  const cc = cfMeta.country || "XX";
  ...
  const ccMap = {
    ...
    "TR": "土耳其", "UA": "乌克兰", "AE": "阿联酋", "SA": "沙特", "ZA": "南非"
  };

  const cityMap = {
    "tokyo": "东京", "osaka": "大阪", "nagoya": "名古屋", "fukuoka": "福冈",
    "hong kong": "香港", "hongkong": "香港", "taipei": "台北", "singapore": "新加坡", 
    "seoul": "首尔", "incheon": "仁川", "bangkok": "曼谷", "kuala lumpur": "吉隆坡", 
    "los angeles": "洛杉矶", "san francisco": "旧金山", "new york": "纽约", "seattle": "西雅图", 
    "sanjose": "圣何塞", "san jose": "圣何塞", "santa clara": "圣克拉拉", "chicago": "芝加哥", 
    "miami": "迈阿密", "ashburn": "阿什本", "oregon": "俄勒冈", "dallas": "达拉斯",
    "frankfurt": "法兰克福", "london": "伦敦", "paris": "巴黎", "amsterdam": "阿姆斯特丹",
    "stockholm": "斯德哥尔摩", "dublin": "都柏林", "dubai": "迪拜", "riyadh": "利雅得"
  };

  const cityName = cfMeta.city || "";
  const cnCountry = ccMap[cc.toUpperCase()] || cfMeta.country || "未知";
  ...
  const finalProxy = {
    ip: cfMeta.clientIp || "获取失败",
    loc: `${getFlag(cc)} ${finalLocationString}`.trim(),
    isp: fmtProxyISP(cfMeta.asOrganization || cfMeta.asNumber),
    cc: cc
  };

  // 7. 纯净属性与欺诈风险评估转化
  const isRes = purData?.isResidential;
  ...
  const textVideo = `${fmtUnlock('NF', rNF, finalProxy.cc)}  ${fmtUnlock('DP', rDP, finalProxy.cc)}  ${fmtUnlock('TK', rTK, finalProxy.cc)}`;
  const textAI = `${fmtUnlock('GPT', rGPT, finalProxy.cc)}  ${fmtUnlock('CL', rCL, finalProxy.cc)}  ${fmtUnlock('GM', rGM, finalProxy.cc)}`;
  ...
  return {
    ...
    children: [
      ...
      { type: 'stack', direction: 'row', gap: 10, children: [
          ...
          // 【右边栏】：中转代理出口 (全面接入 Cloudflare 动态流)
          { type: 'stack', direction: 'column', gap: 4.5, flex: 1, children: [
              Row("paperplane.fill", C.mem, "出口", fmtIP(finalProxy.ip), C.text), // 🚀 完美渲染来自 CF 元数据引擎的实时双栈 IP 
